...
public class AllelogramApplet extends Application
{
  ...
  {
    new AllelogramApplet("C:/temp/carl/PM_file_format", "C:/temp/carl/PM_data.txt").run("Allelogram");
  }
  ...
  {
    File file = chooseFile(settingsFile);
    if (file == null)
      return;
    try
    {
  ...
  {
    File file = (settingsFile != null) ? new File(settingsFile) : FileUtil.pickFile();
    if (!file.exists())
    {
      int i = 1;
    }
